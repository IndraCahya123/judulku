...
function KaprodiTable(props) {
  ...
  const { isFetching, refetch } = useQuery(
    'GetAllKaprodiUserData',
    async () => {
      const res = await BaseUrl.get('/staffs/kaprodi')
      setData({
        kaprodi: res.data.data.staff,
      })
    },
    { refetchOnWindowFocus: false },
  )

  const deleteKaprodi = useMutation('DeleteKaprodiCache', async (id) => {
  ...
  const confirmDelete = (id) => {
    ...
    const confirmation = confirm(`Hapus Kepala Program Studi dengan id ${id} ?`)

    if (confirmation) {
      deleteKaprodi.mutate(id)
    } else {
      alert('Kepala Program Studi tidak dihapus')
    }
  }
