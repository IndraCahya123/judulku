...
function KaprodiNav() {
  ...
  const [active, setActive] = useContext(KaprodiNavContext);
  return (
    <div className="link-wrapper d-flex flex-column">
      ...
      <button
        type="button"
        className={`nav-btn d-flex align-items-center mb-3 ${
          active.kaprodiResponse ? "nav-btn-active" : ""
        }`}
        onClick={() => {
          setActive({
            type: "KAPRODI_RES",
      ...
      >
        ...
        />
        <span
          style={active.kaprodiResponse ? { color: "#fff" } : { color: "#000" }}
        >
          Judul
        </span>
      </button>
